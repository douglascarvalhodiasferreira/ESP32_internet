...
if(isset($_GET['n'])){ //verifica se tem solicitação no get
	
	$nome = $_GET['n'];
	$volumeT = $_GET['vt']; // armazena o que veio do get na variavel
	$dia=$_GET['d'];
	$mes=$_GET['m'];
	$ano=$_GET['a'];
	$hora=$_GET['h'];
	...
	$data = date('d/m/Y H:i:s'); //variavel com um timestamp da data e hora
		...
		$sql = "INSERT INTO `volumes_agua`($nome,dia,mes,ano,hora) VALUES (:volumeT,:dia,:mes,:ano,:hora)"; // acessa a tabela temp_aqua para armazenar o valor da temperatura

		$stmt = $PDO->prepare($sql);// manda preparar os dados para gravar

		$stmt->bindParam(':volumeT',$volumeT); //deixa o armazenamento no banco de dados mais seguro
